	...
	<script>
		function gotry() {
			if(document.iu_form.lentno.value == ""){
				...
				document.iu_form.action = "reservation0212_update_process.jsp";
...
<body>
	...
	<%
		...
		try{
			String sql = "select lentno,custname,bookno,to_char(outdate,'yyyy-mm-dd'),to_char(indate,'yyyy-mm-dd'),status,class1 from reservation0212 where lentno=?";
			...
			sql = "select bookname from bookinfo0212 where bookno=?";
			pstmt = conn.prepareStatement(sql);
	...
	<section>
		...
		<form name="iu_form" method="post" action="reservation0212_update.jsp">
			<table id="iu_table">
				<tr>
					<th>대출번호</th>
